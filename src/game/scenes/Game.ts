...
export class Game extends Scene
{
    ...
    private enterRoom (id: number): void
    {
        ...
        if (prevId >= 0 && this.dungeon.wallGroups.has(prevId)) {
            const c2 = this.physics.add.collider(this.player.sprite, this.dungeon.wallGroups.get(prevId)!);
            this.playerColliders.push(c2);
        }
        if (nextId < NUM_ROOMS && this.dungeon.wallGroups.has(nextId)) {
            const c3 = this.physics.add.collider(this.player.sprite, this.dungeon.wallGroups.get(nextId)!);
