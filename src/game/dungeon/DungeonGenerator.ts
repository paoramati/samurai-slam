...
export class DungeonGenerator
{
    ...
    ): void
    {
        ...
        for (let col = 0; col < cols; col++) {
            for (let row = 0; row < rows; row++) {
                ...
                if (isDoorGap) {
                    ...
                    scene.add.image(wx + TILE / 2, wy + TILE / 2, 'floor').setDepth(-2);
                    // Door marker text
                    if ((isLeftWall && leftDoor && row === midRow) ||
                        (isRightWall && rightDoor && row === midRow)) {
                        const arrow = isLeftWall ? '←' : '→';
                        scene.add.text(wx + TILE / 2, wy + TILE / 2, arrow, {
                            fontSize: '18px', color: '#4fc3f7'
                        }).setOrigin(0.5).setDepth(1);
                    }
                } else {
                    // Wall tile
                    scene.add.image(wx + TILE / 2, wy + TILE / 2, 'wall').setDepth(-1);
                    const wallSprite = wallGroup.create(wx + TILE / 2, wy + TILE / 2, 'wall') as Phaser.Physics.Arcade.Sprite;
        ...
        scene.add.text(rx + rw / 2, ry + 20, labelText, {
            fontSize: '14px', color: '#546e7a'
        }).setOrigin(0.5).setDepth(1);
